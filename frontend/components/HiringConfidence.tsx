...
export default function HiringConfidence({ confidence }: HiringConfidenceProps) {
  ...
  const safeConfidence = isAvailable ? confidence : 0;
  ...
  }, [safeConfidence, isAvailable, springValue]);
  ...
  const getRecommendationLabel = (score: number | null | undefined) => {
  ...
  const rec = getRecommendationLabel(isAvailable ? safeConfidence : null);
  ...
  return (
    <div className="glass-card rounded-2xl p-5 border border-white/10 shadow-xl space-y-4 text-center">
        ...
        <div className="relative w-32 h-32 flex items-center justify-center">
          ...
          <div className="absolute flex flex-col items-center justify-center">
            {isAvailable ? (
              ...
              <span className="text-xs font-semibold text-gray-400 px-2 text-center">
                Not yet available
              </span>
            )}
          </div>
      ...
      <div className="p-2.5 rounded-xl bg-slate-900/80 border border-white/5 text-xs text-gray-300 flex items-center justify-center gap-2">
        ...
        <span>Live Evidence Tracking</span>
      </div>
    </div>
  );
}
